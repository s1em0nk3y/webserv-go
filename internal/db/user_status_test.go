...
func TestUserStatus(t *testing.T) {
	...
	status, err := database.GetUserStatus(user)
	if err != nil {
		t.Fatal(err)
	}
	if status.CompletedTaskCount != 1 {
		t.Fatal("task count is not 1")
	}
	if status.Score-award > 0.0000001 {
		t.Fatal("the scores are not eq")
	}
}
